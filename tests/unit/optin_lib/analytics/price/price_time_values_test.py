...
def test_time_value_changes_from_strike_to_atm_distance_atm(df_opt_hist):
...
def test_time_value_changes_from_strike_to_atm_distance_strike_itm(df_opt_hist, atm_strike):
...
def test_time_value_changes_from_strike_to_atm_distance_strike_otm(df_opt_hist, atm_strike):
    df_time_values = time_value_series_by_strike_to_atm_distance(df_opt_hist, strike=atm_strike + 20)
    assert isinstance(df_time_values, pd.DataFrame)
    assert len(df_time_values) > 10
    assert OCl.TIMESTAMP.nm in df_time_values.columns
...
def test_time_value_series_by_atm_distance(df_opt_hist):
