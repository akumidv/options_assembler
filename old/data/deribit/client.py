...
from exception import DeribitAPIException, DeribitRequestException
from typing import Optional, Dict, Union, List, Tuple
import aiohttp
import asyncio

# https://docs.deribit.com/#overview
class BaseClient:
    API_URL = 'https://www.deribit.com/api'
    API_TESTNET_URL = 'https://test.deribit.com/api'
    ...
    def _init_private_key(self, private_key: Optional[Union[str, Path]], private_key_pass: Optional[str] = None):
        if not private_key:
            return
        # if isinstance(private_key, Path): # TODO
...
class AsyncClient(BaseClient):
    def __init__(
        ...
        testnet: bool = False, den=False, loop=None, session_params: Optional[Dict[str, str]] = None,
        ...
        super().__init__(api_key, api_secret, requests_params, base_endpoint, testnet, den, private_key, private_key_pass)
    ...
    async def create(
        ...
        self = cls(api_key, api_secret, requests_params, base_endpoint, testnet, den, loop, session_params)
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse):
        ...
        if not str(response.status).startswith('2'):
            raise DeribitAPIException(response, response.status, await response.text())
        try:
            return await response.json()
        except ValueError:
            txt = await response.text()
            raise DeribitRequestException(f'Invalid Response: {txt}')
