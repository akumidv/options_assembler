...
- Details and other operational gotchas:
  [_aitna/memory/env-and-test-running.md](memory/env-and-test-running.md).
...
- When adding a capability an assistant will reuse, package it as code under the project
  ...
  [_aitna/tools/](tools/) — do not rely on re-deriving it through ad-hoc steps.
- Reuse existing knowledge: consult [skills/](../../skills/) (USAGE — domain concept→function map) and
  [_aitna/memory/](memory/) before re-researching from scratch.
...
1. **Describe + plan, then save *first*.** Before writing code, record the task in
   [`_aitna/TASKS.md`](TASKS.md): a stable id/title, the **goal**
...
> The general rule lives in the akmon guardrail
> [`API shape — subject first`](../_aitna/akmon/guardrails/_common.md#api-shape--subject-first)
