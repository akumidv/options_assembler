...
def test_add_atm_itm_otm(df_ext_brn_hist):
    dt_filter = df_ext_brn_hist[OCl.DATETIME.nm].max() - datetime.timedelta(days=10)
    df_opt_ext = price.add_atm_itm_otm_by_chain(df_ext_brn_hist[df_ext_brn_hist[OCl.DATETIME.nm] > dt_filter])
    assert isinstance(df_opt_ext, pd.DataFrame)
...
def test_add_atm_itm_otm_exp(df_ext_brn_hist):
    dt_filter = df_ext_brn_hist[OCl.DATETIME.nm].max() - datetime.timedelta(days=10)
    df_opt_ext = price.add_atm_itm_otm_exp(df_ext_brn_hist[df_ext_brn_hist[OCl.DATETIME.nm] > dt_filter])
    assert isinstance(df_opt_ext, pd.DataFrame)
    assert OCl.PRICE_STATUS.nm in df_opt_ext.columns
    assert not df_opt_ext[df_opt_ext[OCl.PRICE_STATUS.nm] == OptionPriceStatus.ATM.code].empty
