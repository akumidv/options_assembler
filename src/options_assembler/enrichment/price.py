...
import pandas as pd
from options_assembler.entities import OptionColumns as OCl, OptionType, OptionPriceStatus
from options_assembler.chain import (
...
def add_intrinsic_and_time_value(df_hist):
    ...
    df_hist.loc[df_hist[OCl.OPTION_TYPE.nm] == OptionType.CALL.code, OCl.INTRINSIC_VALUE.nm] = \
        df_hist[OCl.UNDERLYING_PRICE.nm] - df_hist[OCl.STRIKE.nm]
    df_hist.loc[df_hist[OCl.OPTION_TYPE.nm] == OptionType.PUT.code, OCl.INTRINSIC_VALUE.nm] = \
...
def add_atm_itm_otm_exp(df_hist):
    ...
    def atm_otm_itm(x):
        ...
        if x.iloc[0][OCl.OPTION_TYPE.nm] == OptionType.CALL.code:
