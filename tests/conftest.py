...
@pytest.fixture(name='df_brn_hist')
def fixture_df_brn_hist(option_symbol, exchange_provider, provider_params):
    """Option dataframe"""
    if _CACHE.get('DF_BRN_OPT') is None:
        _CACHE['DF_BRN_OPT'] = exchange_provider.load_option_history(symbol=option_symbol, params=provider_params,
                                                                     columns=exchange_provider.option_columns)
    return _CACHE['DF_BRN_OPT'].copy()


@pytest.fixture(name='df_brn_fut')
def fixture_df_brn_fut(option_symbol, exchange_provider, provider_params):
    """Future dataframe"""
    if _CACHE.get('DF_BRN_FUT') is None:
        _CACHE['DF_BRN_FUT'] = exchange_provider.load_future_history(symbol=option_symbol, params=provider_params,
                                                                     columns=exchange_provider.future_columns)
    return _CACHE['DF_BRN_FUT'].copy()


@pytest.fixture(name='df_ext_brn_hist')
def fixture_df_ext_brn_hist(df_brn_hist, df_brn_fut):
    """Option dataframe with future"""
    if _CACHE.get('DF_EXT_BRN_OPT') is None:
        _CACHE['DF_EXT_BRN_OPT'] = join_option_with_future(df_brn_hist, df_brn_fut)
    return _CACHE['DF_EXT_BRN_OPT'].copy()


@pytest.fixture(name='df_brn_chain')
